...
translations.ru = translations.ru || {
  ...
  "seo.description":
    "MITE — промышленная IIoT-платформа, которая подключает активы, моделирует бизнес-логику и превращает операционные данные в решения.",

  // HERO
  "hero.kicker": "Промышленный IoT — в продакшн",
  "hero.title": "Не просто ещё одна IIoT-платформа. <span>MITE меняет правила игры.</span>",
  "hero.subtitle":
    "От сырой телеметрии к реальному исполнению: подключайте активы, описывайте логику — и пусть MITE управляет операциями. " +
    "Каждое выполненное действие становится данными — подпитывая следующее решение, правило и улучшение.",
  "hero.primaryCta": "Запросить демо",
  "hero.secondaryCta": "Посмотреть платформу",
  "hero.meta1": "От пилотов до продакшна — готовые внедрения",
  "hero.meta2": "Декларативная логика — описываете что делать, а не как кодить",
  "hero.meta3": "Любая сложность логики • любая промышленная реальность • быстрые итерации",
  "hero.badge": "<strong>«Просто, понятно и надёжно — ровно то, чего ждут промышленные команды.»</strong>",
  ...
  aboutOutcomes: [
    {
      ...
      bullets: [
        "Единый источник правды по исполнению — данные превращаются в инсайты",
        ...
        "Итог: решения опираются на наблюдаемое поведение системы, а не на предположения"
      ]
    },
    {
      num: "Дни",
  ...
  "platform.subtitle":
    "Вы описываете логику как правила и процессы. Платформа исполняет их последовательно — на масштабе.",

  "platform.cards.0.title": "Любая сложность процесса — без кода. И это реально работает в исполнении.",
  "platform.cards.0.text":
    "Моделируйте процессы как логику: условия, маршрутизация, согласования, эскалации, KPI. " +
    "Опишите, что должно произойти — MITE обеспечит исполнение, согласованность и трассируемость. " +
    "Сложность может расти, не превращаясь в хаос кастомного кода.",
  "platform.cards.0.meta": "Меньше трения с dev • быстрее итерации • яснее ответственность",
  ...
  "platform.story.text":
    ...
    "Поэтому улучшения приезжают за часы или дни — и поэтому платформа " +
  ...
  "highlights.subtitle":
    "Структурированная «песочница данных» плюс инструменты, чтобы быстро строить любую контрольную логику и операционный опыт.",
  ...
  "useReal.subtitle":
    "Сейчас — короткие карточки (доверие). Позже можно расширить каждый кейс в отдельную страницу с механикой, доказательствами и скриншотами.",
  ...
  "how.steps.0.text":
    "Собираем и нормализуем вашу реальность (за дни). Поставщики отправляют телеметрию на ваш endpoint — мы принимаем её и превращаем сырые события в чистую, доступную для запросов операционную модель.",
  "how.steps.0.output":
  ...
  "how.steps.2.text":
    "Исполнение → измерение → улучшение. MITE непрерывно крутит цикл: запускает действия, хранит доказательства, отслеживает влияние на KPI и помогает безопасно итераировать — без поломки продакшна.",
  "how.steps.2.output":
  ...
  "pricing.plan2.name": "Scale",
  "pricing.plan2.tagline": "Разворачивайтесь по площадкам с предсказуемой поддержкой.",
  ...
  "testimonials.items.1.text":
    "«Во время внедрения мы меняли логику каждую неделю — без тикетов разработчикам. Поменялись операции — сразу поменялась система.»",
  ...
  "testimonials.items.3.text":
    "«Масштабирование стало предсказуемым: одна модель данных, одни механизмы — разные площадки. Никаких ‘снежинок-интеграций’.»",
  ...
  "faq.items.3.a":
    "Если требование улучшает платформу системно (а не является узким разовым случаем), мы обычно обобщаем его и быстро поставляем. " +
    "Это значит: вы не платите за изолированную кастомную «разводку труб» — а платформа становится сильнее с каждым реальным внедрением. " +
  ...
  "contact.title": "Скажите, что вы хотите контролировать — мы наметим первый быстрый win",
  "contact.subtitle":
  ...
  useCases: [
    {
      industry: "pharma",
      industryLabel: "Фарма",
      title: "Холодовая цепь: перестаньте терять партии из-за контроля температуры «после факта»",
    ...
    },
    {
      ...
      pain:
        "Данные EM есть — но они разрозненны: тренды BMS, отдельные сенсоры, калибровки, контроль доступа, ручные сменные записи. " +
        "Когда происходит отклонение (T/RH/DP/частицы), «расследование» — это поиск данных: кто заходил, что менялось, какая комната, какое окно экспозиции. " +
        "QA собирает доказательства постфактум, а root cause часто сводится к «человеческому фактору», потому что восстановить полный таймлайн слишком сложно. " +
        "Итог: медленные разборы, повторяемые отклонения и подготовка к аудиту как марафон бумажной работы.",
      how:
    ...
    },
    {
      ...
      title: "Микроостановки: перестаньте платить за простои без root cause",
      kpiBadge: "↓ Микроостановки 15–30% · ↑ OEE на 2–6 п.п. · ↓ «пинг-понг обвинений»",
      ttvBadge: "Пилот за 2–4 недели",
      pain:
        "Линия «работает», но деньги утекают в остановках по 3–7 минут весь день. Ремонт говорит «операторы», операторы — «оборудование», менеджмент — «вариабельность». " +
        "PLC-сигналы в одном месте, сменные записи — в другом, заявки в ремонт — в третьем. " +
        "Поэтому каждую микроостановку объясняют по-разному — и улучшений нет, потому что цепочка исполнения за каждой остановкой невидима.",
      how:
        "MITE выравнивает состояния машин, входной поток, алармы, действия оператора и контекст материалов в одну операционную модель и таймлайн. " +
        "Правила детектируют повторяющиеся подписи остановок (последовательность + длительность + пред-условия). " +
        "AI кластеризует похожие события и подсвечивает самые стабильные предикторы (нестабильная подача, дрейф после переналадки, шум сенсора vs реальный процесс). " +
        "Доступ к данным по людям — под GDPR-контролем (RBAC, лог экспорта, настраиваемая ретенция; при необходимости — псевдонимизация).",
      result:
        "Микроостановки превращаются в объяснимые паттерны, а не «случайность». Заводы обычно режут микроостановки на 15–30% и получают +2–6 п.п. OEE на том же железе. " +
        "Обсуждения смещаются от мнений к доказательствам — и фиксы держатся, потому что триггеры измеримы.",
      icon: "factory",
