...
"platform.cards.0.meta":
  ...
  useCases: [
    {
      ...
      pain:
        ...
        "В итоге отклонения находят, когда товар уже испорчен, и каждый инцидент превращается в поиск в 3–5 системах + письма + ручную сверку (4–8 часов).",
      how:
        "MITE не заставляет «оснастить датчиками всё». Мы начинаем с того, что собираем то, что уже существует, и объединяем это в единую модель как источника правды. " +
      ...
      result:
        "Отклонения обрабатываются «на лету», а не превращаются в списание в брак. " +
    ...
    },
    {
      ...
      ttvBadge: "Пилота до рабочей модели 2–3 недели",
      pain:
  ...
  "Когда происходит отклонение (температура, влажность, перепад давления, частицы), расследование превращается в поиск данных: кто заходил, что менялось, какая комната и какое время работы с партией. " +
  ...
  "В итоге разбор занимает часы или дни, отклонения повторяются, а подготовка к аудиту как марафон бумажной работы.",
      how:
        "MITE сначала собирает то, что уже есть (EM/BMS/доступ/циклы оборудования) и объединяем в одну аудито-дружелюбную модель. " +
        "Сигналы + состояния помещений + события дверей/доступа + время работы с партией становятся единым управляемым таймлайном. " +
        ...
        "AI подсвечивает вероятные драйверы и повторяющиеся паттерны по комнатам/сменам — расследование начинается с целостной картины зафиксированных цепочек действий и сигналов.",
      result:
        ...
        "И когда вы решите добавить новые сенсоры позже — они просто дополняют и обогощают ту же модель без переразработки.",
      icon: "pharma",
    ...
    },
    {
      industry: "manufacturing",
        ...
        ttvBadge:
        "Первый рабочий сценарий за 30ч/ч",
        ...
        "Это позволяет увидеть реальные причины — например нестабильную подачу материала, микро сбои на линии, нетипичную вибрацию оборудования или ошибки после переналадки. " +
        ...
        "Заводы обычно снижают их количество на 15–30% и получают рост OEE на 2–6% без модернизации оборудования. " +
    ...
    },
    {
      industry: "manufacturing",
      industryLabel: "Производство",
      title: "Энергопотери: перестаньте платить за машины, которые ничего не делают",
      kpiBadge: "↓ Холостое потребление 10–20% · ↓ Пиковые штрафы · ↑ Маржа на единицу",
      ttvBadge: "Результат через 2 недели",
      pain:
    ...
    },
    {
      ...
      pain:
        ...
        "Параметры процесса, среда и реальные действия живут в разных системах — корреляции теряются. " +
      ...
      how:
        ...
        "Правила подсвечивают рискованные комбинации параметров и отклонение параметров за допустимые пределы. " +
        "AI находит повторяемые взаимодействия (например, параметры материалов + данные с датчиков + паттерн нормального поведения процесса) и предлагает «ограждения» до появления дефектов. " +
        "GDPR: ролевой доступ, лог экспорта, политики хранения.",
      result:
        "Команды переходят от реакции на дефект к предотвращению в моменте. Типовые внедрения снижают брак на 10–25% и улучшают долю продукции без переделки. " +
        "Причины определяются по факту, потому что система хранит полный контекст — а не только финальный код дефекта.",
      icon: "wrench",
    ...
    },
    {
      ...
      how:
        "MITE отслеживает перемещения материалов, триггеры расхода и регистрации партий в одном таймлайне исполнения. " +
    ...
    },
    {
      ...
      result:
        "Обычно маржа растёт на 5–12% без дополнительных инвестиций — за счёт удаления повторяющихся потерь исполнения. " +
