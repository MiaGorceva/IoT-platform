...
const translations = {
  ...
  // RU / UK можно “подогнать переводами” позже — главное, чтобы не было дублей и синтаксических ошибок.
  // Сейчас оставляю RU/UK как fallback = EN (чтобы ничего не ломалось).
  ru: {},
  uk: {}
};

// fallback: если ru/uk пустые — берём en и подменяем только те ключи, которые есть
